...
public class World {
	...
	private ArrayList<GameObject> objects; // Liste aller im Spiel aktiven
											// Objekte
	private float speed; // Spielgeschwindigkeit in (% der Darstellungsbreite
							// pro Sek.)
	private ObjectGenerator objectGen; // der Objekt-Generator, welcher die
										// Spielwelt erzeugt (wird im
	...
	private float distance; // die zurückgelegte Strecke - legt Geschwindigkeit
							// fest
	private int score; // Anzahl des gesammelten Mülls
	private BitmapFont font; // Schriftart zum schreiben

	public World(ObjectGenerator objectGen, float iniSpeed, GameState state,
			BitmapFont font) {
	...
	public void draw(Batch batch, boolean android) { // Alle Spielobjekte
														// zeichnen
	...
	public void update(float deltaTime) {
		// Diver auf Standardgeschwindigkeit (nachdem er verlangsamt wurde durch
		// kollision)
		diver.refresh();
		...
		if (o != null) {
			// if diver collects trash update score
			if (o.getType() == ObjectType.TRASH) {
				o.delete();
				score += o.getTrashScore();
			}
			// if diver collides with shark game over
			if (o.getType() == ObjectType.SHARK) {
				state.gameOver();
				// if diver swims through plants reduced speed
			} else if (o.getType() == ObjectType.PLANT) {
				diver.slow();
				// if diver collides with jellyfish reduced speed, substract air
		...
		// update air
		if (diver.getShape().getY() + diver.getShape().getHeight() >= 950) {
			diver.recover();
		}
